...
class TinyTag(object):
    ...
    def load(self, filehandler, tags, length, encoding):
        if tags:
            self._parse_tag(filehandler, encoding)
            filehandler.seek(0)
        if length:
...
class ID3V2(TinyTag):
    ...
    def __init__(self, filehandler, tags=True, length=True, encoding='utf-8'):
        TinyTag.__init__(self)
        self.load(filehandler, tags=tags, length=length, encoding=encoding)
    ...
    def _parse_tag(self, fh, encoding):
        self._parse_id3v2(fh, encoding)
        if not self.has_all_tags():  # try to get more info using id3v1
            fh.seek(-128, 2)
            self._parse_id3v1(fh, encoding)

    def _parse_id3v2(self, fh, encoding='ISO-8859-1'):
        header = struct.unpack('3sBBB4B', fh.read(10))
        tag = codecs.decode(header[0], encoding)
    ...
    def _parse_id3v1(self, fh, encoding='ASCII'):
        if fh.read(3) == b'TAG':  # check if this is an ID3 v1 tag
            asciidecode = lambda x: self._unpad(codecs.decode(x, encoding))
